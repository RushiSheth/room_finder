...
class _MyPropertyState extends State<MyProperty> {
  ...
  List<Property> myPropertyList = List<Property>();
  List<Property> propertyList = List<Property>();
  Future<List<Property>> getMyPropertyList(int i) async {
    SharedPreferences  preferences =  await SharedPreferences.getInstance();
    int userId = preferences.getInt('userId');
    if(myPropertyList.isEmpty){
      propertyList = await getProperties();
      for(i=0;i<propertyList.length;i++)
        {
          if(propertyList[i].userid==userId)
            {
              myPropertyList.add(propertyList[i]);
            }
        }
    ...
    // else if(i == 0)
    // {
    //   myPropertyList = await getProperties();
    // }
    return myPropertyList;
                    ...
                    builder: (BuildContext context, AsyncSnapshot snapshot){
                      if(snapshot.data==null){
                          ...
                          child: Center(
                            child: Column(
                              ...
                              children: <Widget>[
                                ...
                                SizedBox(height: 10,),
                                Text('Fetching Your Properties',style: Theme.of(context).textTheme.overline.copyWith(fontSize: 12),)
                              ],
                            ),
                          ),
                        );
                        ...
                        return Container(
                          child: Center(
                            child: Column(
                              ...
                              mainAxisAlignment: MainAxisAlignment.center,
                              children: <Widget>[
                                Text('No Properties Added',style: Theme.of(context).textTheme.overline.copyWith(fontSize: 12),)
                              ],
                            ),
                          ),
                        );
                        ...
                        return Column(
                          children: <Widget>[
                            Container(
                              child: Text('Properties: '+snapshot.data.length.toString()),
